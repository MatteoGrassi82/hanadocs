...
description: "Connect HANA to 150+ Electronic Health Record systems. Bidirectional data flow — read patient context, write clinical notes back."
---
...
### 2. Integration Middleware (Redox, Catagon)

For EHR systems without open APIs, or when you need a standard interoperability layer.

| Middleware | EHRs Supported | Setup Time | Notes |
|-----------|---------------|------------|-------|
| Redox | Athena, Epic, Cerner, 200+ | 2-4 weeks | HL7 FHIR, production-ready |
| Catagon | Multiple EU/US EHRs | 2-4 weeks | Zapier-like EHR connector |

**How it works:**

```
Your EHR ←→ Redox/Catagon ←→ HANA Connector ←→ HANA Reasoning Engine
```

<Note>
Middleware platforms like Redox carry their own licensing costs (typically $15K-25K platform fee + per-connector fees). HANA works with your existing Redox subscription if you have one.
</Note>
...
<Steps>
  <Step title="Identify EHR and API Availability">
    What EHR does the organization use? Does it have an open API? Is Redox/Catagon already in use?
  </Step>
