...
```mermaid
graph TB
    Q["Clinical Query\n+ Patient Context"] --> TM["Triage Model"]

    TM -->|"Task Type + Specialties"| E1["Expert Model 1"]
    TM -->|"Task Type + Specialties"| E2["Expert Model 2"]
    TM -->|"Task Type + Specialties"| EN["Expert Model N"]

    E1 -->|"Response + P(X)"| AGG["Deliberation\nAggregation"]
    E2 -->|"Response + P(X)"| AGG
    EN -->|"Response + P(X)"| AGG

    AGG --> WLOP["WLOP Probability\nWeighting"]
    WLOP --> CB["Cascade\nBoosting"]
    CB --> DM["Deliberation\nModel"]
    DM --> PLAN["Final Conversation Plan\nQuestions · Branching · Escalation"]
```
